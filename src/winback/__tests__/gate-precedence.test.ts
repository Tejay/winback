...
describe('scheduleExitEmail — gate precedence', () => {
  ...
  it('classifier handoff: subscriber email is NOT sent, no email_sent row', async () => {
    // Gates 1–4 clear, then the handoff branch fires.
    queueRow({ dnc: false })
    queueRow({ pausedAt: null })
    queueRow({ activatedAt: null, stripeSubscriptionId: null, pilotUntil: null })
    queueRow({ aiPausedUntil: null })
    // triggerFounderHandoff: select the subscriber row.
    queueRow({
      id: 'sub_handoff',
      customerId: 'cust_1',
      email: 'sub@example.com',
      ...
      planName: 'Pro',
      mrrCents: 9900,
      cancellationReason: 'enterprise pricing',
      triggerNeed: '',
      cancelledAt: new Date('2026-04-01'),
      stripeComment: 'need enterprise pricing discussion',
      replyText: '',
      founderHandoffAt: null,
    })

    const handoffClassification: ClassificationResult = {
      ...baseClassification,
      handoff: true,
      handoffReasoning: 'Asked for enterprise pricing — founder territory',
      recoveryLikelihood: 'high',
    }

    await scheduleExitEmail({
      subscriberId: 'sub_handoff',
      email: 'sub@example.com',
      classification: handoffClassification,
      fromName: 'Alex',
    })

    // The subscriber-facing win-back email must NOT be inserted into wb_emails_sent.
    expect(mockInsert).not.toHaveBeenCalled()

    // The founder handoff path may or may not call resend.emails.send depending
    // on whether a recipient resolves; we mocked resolveFounderNotificationEmail
    // to return null, so no notification email goes out either. Either way,
    // sendEmail (which would go to sub@example.com with the win-back subject)
    // is the thing we care about NOT happening:
    const subscriberSendCalls = mockSend.mock.calls.filter(
      (call: unknown[]) =>
        (call[0] as { to?: string })?.to === 'sub@example.com',
    )
    expect(subscriberSendCalls).toHaveLength(0)

    // founder_handoff_triggered event is logged.
    expect(mockLogEvent).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'founder_handoff_triggered' }),
    )
  })
})
