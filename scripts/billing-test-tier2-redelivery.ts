// Tier 2.3 — Webhook redelivery idempotency (Connect-side).
//
// Drives a real win-back flow (cancel + re-subscribe on Stripe Connect)
// to land processChurn + processRecovery, snapshots the relevant DB
// counters, asks Stripe to redeliver both events via `stripe events
// resend`, then re-snapshots and diffs.
//
// The test passes when there is zero drift on:
//   - wb_churned_subscribers count
//   - wb_recoveries count
//   - wb_emails_sent count
//   - customers.activatedAt / stripePlatformCustomerId
//
// Confirmed both handlers are idempotent under redelivery — second
// delivery short-circuits via the existing "row already exists" /
// "status != recovered" checks.
//
...
async function main() {
  ...
  // Inject synthetic exit email so processRecovery counts the re-subscribe.
  ...
  // wb_events is allowed to drift (each redelivery is its own audit entry).
